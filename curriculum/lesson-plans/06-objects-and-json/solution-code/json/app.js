...
//  Artist: Artist name
//  Units sold: 31234
let grungeAlbumsJS = JSON.parse(grungeAlbumsJSON);

for(let i=0; i< grungeAlbumsJS["albums"].length; i++){
  let currentAlbum = grungeAlbumsJS["albums"][i];
  console.log("%s, %s, Units sold: %d",currentAlbum["name"],currentAlbum["artist"],currentAlbum["unitsSold"]);
  //let albumTemplate="<li><h4><strong>" + currentAlbum["name"] + "</strong> by " + currentAlbum["artist"] + "</h3><span>Units sold : " + currentAlbum["unitsSold"] + "</span></li>";
...
let instrumentsJSON = '{"instruments": ["guitar", "bass", "drums"]}';
let instruments = JSON.parse(instrumentsJSON);
